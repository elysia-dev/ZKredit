...
export const setRequestERC721Task = task("setRequestERC721", "set request for ERC721")
  .addOptionalParam("requestId", "request id", "1")
  .setAction(
    async (taskArgs, hre) => {
      ...
      if (requestId === 1) {
        ...
        // query for credit score > 900
        query = {
          schema: schemaBigInt,
          claimPathKey  : schemaClaimPathKey,
          operator: Operators.GT, // operator
          value: [900, ...new Array(63).fill(0).map(i => 0)], // for operators 1-3 only first value matters
        };
      } else if (requestId === 2) {

      } else {
        console.log("requestId must be 1 or 2")
        return
      }
